...
export default function SnippetEditor() {
  ...
  const [folder, setFolder] = useState(
    existing?.folder || "Frontend Frameworks",
  );
  const [language, setLanguage] = useState(
    existing?.language || "Tailwind CSS",
  );
  ...
  const [isCustomFolder, setIsCustomFolder] = useState(() => {
    ...
    return existing ? !defaultFolders.includes(existing.folder) : false;
  });
  ...
  const existingFolders = useMemo(() => {
    ...
    const foldersSet = new Set<string>(defaultFolders);
    snippets.forEach((s) => {
  ...
  useEffect(() => {
    // If the snippet doesn't exist, navigate away.
    // Avoid setting local state here — initializers already use `existing`.
    if (!existing) {
  ...
  const handleSave = () => {
    ...
    // Using static `mockSnippets` for now — no persistence.
    router.push("/snippets");
  };
    ...
    <div className="flex flex-col h-full bg-[#07090d] select-none min-h-screen text-[#eceff4]">
      ...
      <header className="bg-workspace-bg/95 backdrop-blur-xl border-b border-[#2d303b]/40 flex justify-between items-center w-full px-6 md:px-8 h-20 shrink-0 z-30 select-none">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="hover:bg-[#181c26] p-2.5 rounded-xl transition-all border border-[#2d303b]/30 flex items-center justify-center text-on-surface active:scale-95"
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="px-4.5 py-2.5 border border-[#2d303b] hover:bg-[#181c26] rounded-xl transition-all text-[#eceff4]/80 text-xs font-semibold h-11 flex items-center justify-center active:scale-95"
          >
            Cancel
          </button>
          <button
            ...
            className="px-5 py-2.5 bg-[#7ca1ff] hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed text-workspace-bg font-sans font-bold text-xs rounded-xl transition-all h-11 flex items-center justify-center gap-1.5 shadow-[0_4px_16px_rgba(124,161,255,0.15)] active:scale-95"
          >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-12 gap-6 pb-20">
          {/* LEFT COLUMN: Input form parameter controls (5 elements) */}
              ...
              <div className="bg-[#12141c]/80 px-5 py-3.5 flex justify-between items-center border-b border-[#2d303b]/40 select-none">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    onClick={handleCopy}
                    disabled={!code.trim()}
                    className="px-3 py-1 hover:bg-[#181c26] border border-[#2d303b]/60 rounded-lg text-[#eceff4]/90 hover:text-[#7ca1ff] duration-150 font-mono text-[11px] font-bold flex items-center gap-1.5 active:scale-95"
                  >
                    {copied ? (
          ...
          {/* RIGHT SIDE PANEL: Metadata, flags, tags controls (3 Cols) */}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tags.map((t) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => removeTag(t)}
                        className="text-on-surface-variant/40 group-hover:text-error transition-colors font-sans font-bold select-none"
                        title="Remove tag"
                      >
                        &times;
                      </button>
